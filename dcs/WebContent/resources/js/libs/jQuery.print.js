...
(function($) {
		...
		try {
			setTimeout(
					function() {
						...
						frameWindow.focus();
						try {
							// Fix for IE11 - printng the whole page instead of
							// the iframe content
							if (!frameWindow.document.execCommand('print',
									false, null)) {
								// document.execCommand returns false if it
								// failed
	...
	$.print = $.fn.print = function() {
		...
		// Notify with generated markup & cloned elements - useful for logging,
		// etc
		try {
		...
		if (options.iframe) {
			...
			try {
				...
				wdoc.write(content);
				wdoc.close();
				printFrame(w, options.timeout).done(function() {
					// Success
					setTimeout(function() {
						// Wait for IE
						if (iframeCount === 0) {
				...
				// Use the pop-up method if iframe fails for some reason
				console
						.error("Failed to print from iframe", e.stack,
								e.message);
				printContentInNewWindow(content, options.timeout).always(
						function() {
							try {
								options.deferred.resolve();
							} catch (err) {
