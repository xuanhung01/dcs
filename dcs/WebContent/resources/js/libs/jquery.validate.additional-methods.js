...
}, "Yêu cầu nhập định dạng số nguyên");
					...
					if ($(element).attr("type") === "file") {
						...
						if (element.files && element.files.length) {
							for (i = 0; i < element.files.length; i++) {
								...
								// Grab the mimetype from the loaded file,
								// verify it matches
...
jQuery.validator.addMethod("vnDate", function(value, element) {
	// FIXME: Use datepicker.parseDate
	try {
	 ...
	 * 
	 * if (value.match(dateformat)) { var seperator1 = value.split('/'); var
	 * seperator2 = value.split('-'); var splitdate = [];
	 * 
	 * if (seperator1.length > 1) { splitdate = value.split('/'); } else if
	 * (seperator2.length > 1) { splitdate = value.split('-'); }
	 * 
	 * var dd = parseInt(splitdate[0]); var mm = parseInt(splitdate[1]); var yy =
	 * parseInt(splitdate[2]); var listofDays = [ 31, 28, 31, 30, 31, 30, 31,
	 * 31, 30, 31, 30, 31 ];
	 * 
	 * if (mm == 1 || mm > 2) { if (dd > listofDays[mm - 1]) { return false; } }
	 * if (mm == 2) { var lyear = false; if ((!(yy % 4) && yy % 100) || !(yy %
	 * 400)) { lyear = true; } if ((lyear == false) && (dd >= 29)) { return
	 * false; } if ((lyear == true) && (dd > 29)) { return false; } } } else {
	 * console.log("Dont match regex! <" + value + ">"); return false; } return
	 * true;
	 */
}, "Yêu cầu nhập định dạng ngày dd/mm/yyyy.");
...
jQuery.validator.addMethod("CheckUTF8", function(value, element) {
	var regexUTF8 = "^[a-zA-Z0-9_.-]*$";
	if (value.match(regexUTF8)) {
		return true;
	} else {
		return false;
	}
}, "Username không được dùng tiếng việt");
...
$.validator.addMethod("dateLessThan",
		function(value, element, params) {
			...
			if ((target == null || target == '') && this.optional(element)){
				return true;
			}
			
//			if (target == null || target == ''
//					|| !/Invalid|NaN/.test(new Date(target))) {
//				return true;
//			}
			//if (!/Invalid|NaN/.test(new Date(value))) {
				try{
					//console.log(value);
					//console.log(target);
					var date1 = $.datepicker.parseDate('dd/mm/yy', value);
					var date2 = $.datepicker.parseDate('dd/mm/yy', target);
...
$.validator.addMethod("dateBeforeToday", function(value, element, params) {
	if (!/Invalid|NaN/.test(new Date(value))) {
		var result = new Date(value) >= new Date();
		if (result) {
			$(params).valid();
		}
		return result;
